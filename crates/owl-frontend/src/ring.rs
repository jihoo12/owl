...
use owl_kernel::nbe::nbe_eval_ctx;
use owl_kernel::session::Session;
use owl_kernel::syntax::{Datatype, Term, shift};
use owl_kernel::typechecker::{Ctx, TypeError, check_dt, infer_dt};
use std::sync::Arc;
        ...
        Mode::Structured => {
            ...
            if let Term::TApp(outer, inner) = t {
                if let Term::TApp(g, one_t) = outer.as_ref() {
                    let g_nf = owl_kernel::nbe::nbe_eval_ctx(r.ctx_len, g, session);
                    let add_nf = owl_kernel::nbe::nbe_eval_ctx(r.ctx_len, &r.add, session);
            ...
            let nf = owl_kernel::nbe::nbe_eval_ctx(r.ctx_len, t, session);
            if nf == r.zero {
                return Some(0);
            }
            if nf == r.one {
...
fn is_add_call(r: &Ring, t: &Term, session: &mut Session) -> bool {
    match t {
        Term::TApp(_, _) => {
            let nf = owl_kernel::nbe::nbe_eval_ctx(r.ctx_len + 1, t, session);
            is_addshape_elim(&nf)
        }
...
fn raw_as_binop(r: &Ring, op: &Term, t: &Term, session: &mut Session) -> Option<(Term, Term)> {
    if let Term::TApp(outer, b) = t {
        if let Term::TApp(g, a) = &**outer {
            ...
            let g_nf = owl_kernel::nbe::nbe_eval_ctx(r.ctx_len, g, session);
            let op_nf = owl_kernel::nbe::nbe_eval_ctx(r.ctx_len, op, session);
            if g_nf == op_nf {
...
pub(crate) fn as_add(r: &Ring, t: &Term, session: &mut Session) -> Option<(Term, Term)> {
    match r.mode {
        Mode::Concrete => {
            let nf = match t {
                Term::TApp(_, _) => owl_kernel::nbe::nbe_eval_ctx(r.ctx_len, t, session),
                _ => t.clone(),
            };
        ...
        Mode::Structured => {
            ...
            match nf {
                Term::TApp(outer, b) => match outer.as_ref() {
                    Term::TApp(g, a) => {
                        let add_nf = owl_kernel::nbe::nbe_eval_ctx(r.ctx_len, &r.add, session);
                        if owl_kernel::nbe::nbe_eval_ctx(r.ctx_len, g, session) == add_nf {
    ...
    match r.mode {
        ...
        Mode::Structured => {
            ...
            match nf {
                Term::TApp(outer, b) => match outer.as_ref() {
                    Term::TApp(g, a) => {
                        let mul_nf = owl_kernel::nbe::nbe_eval_ctx(r.ctx_len, &r.mul, session);
                        if owl_kernel::nbe::nbe_eval_ctx(r.ctx_len, g, session) == mul_nf {
                            Some((a.as_ref().clone(), b.as_ref().clone()))
                        } else {
                            None
                        }
...
pub(crate) fn decomp(
    ...
    owl_kernel::debug_log!("decomp: {}", owl_kernel::syntax::pretty::show_term(&[], t));
...
pub fn prove(
    ...
    if let Some(raw) = raw_goal_ty {
        owl_kernel::debug_log!(
            "ring: RAW sides u={} v={}",
            match raw {
                Term::TPath(_, a, _) => owl_kernel::syntax::pretty::show_term(&[], a),
                _ => "<not-path>".into(),
            },
            match raw {
                Term::TPath(_, _, b) => owl_kernel::syntax::pretty::show_term(&[], b),
                _ => "<not-path>".into(),
            }
        );
    }
    ...
    let prev_skip = owl_kernel::typechecker::termination::should_skip_guard(session);
    owl_kernel::typechecker::termination::set_skip_guard(true, session);
    let check_res = if owl_kernel::debug::is_active() {
        check_dt(dts, ctx, &pf.p, goal_ty, session)
    } else {
        Ok(())
    };
    owl_kernel::typechecker::termination::set_skip_guard(prev_skip, session);
    if let Err(e) = check_res {
        let detail = match &e {
            owl_kernel::typechecker::TypeError::TypeMismatch {
                expected, got, pos, ..
            } => format!(
                "  expected : {}\n  got      : {}\n  pos      : {:?}",
                owl_kernel::syntax::show_term(
