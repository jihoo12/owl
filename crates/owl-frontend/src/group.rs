...
use crate::ring::{EqP, app, inst, refl2};
...
fn tr(g: &Group, p: &EqP, q: &EqP) -> EqP {
    if p.b != q.a {
        owl_kernel::debug_log!(
            ...
            owl_kernel::syntax::pretty::show_term(&[], &p.b),
            owl_kernel::syntax::pretty::show_term(&[], &q.a)
        );
    }
...
pub fn prove(
    ...
    if owl_kernel::debug::is_active() {
        ...
        if let Err(e) = r {
            owl_kernel::debug_log!(
                ...
                owl_kernel::syntax::pretty::show_term(&[], &pu.p)
            );
            return Err(TypeError::Other(format!(
    ...
    if let Err(_e) = &check_res {
        owl_kernel::debug_log!(
            ...
            owl_kernel::syntax::pretty::show_term(&[], goal_ty),
            owl_kernel::syntax::pretty::show_term(&[], &pf.p)
        );
    }
...
fn show_side(t: &Term) -> String {
    owl_kernel::syntax::pretty::show_term(&[], t)
}
