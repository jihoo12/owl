...
use super::*;
use crate::test_helpers::{run_str_test, with_session};
use owl_kernel::interval::I;
use owl_kernel::session::Session;
use owl_kernel::syntax::{LevelExpr, Tactic, Term, show_term};
use std::sync::Arc;
...
fn typecheck_program_rejects_import() {
    owl_kernel::session::with_session_mut(|session| {
...
fn dependent_arrow_type_typechecks() {
    with_session(|session| {
        use owl_kernel::typechecker::infer;
        let ctx = Vec::new();
...
fn id_definition_typechecks() {
    with_session(|session| {
        use owl_kernel::typechecker::{check, infer};
        let ctx = Vec::new();
        let ty = parse_term("∀ (A : U0), A -> A", session).unwrap();
...
fn cumulativity_universe_levels() {
    with_session(|session| {
        use owl_kernel::typechecker::check;
        let ctx = Vec::new();
