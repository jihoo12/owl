...
use owl_kernel::interval::I;
use owl_kernel::syntax::{
...
pub(super) struct Parser {
    ...
    session_ptr: *mut owl_kernel::session::Session,
}

impl Parser {
    pub(super) fn new(tokens: Vec<Token>, session: &mut owl_kernel::session::Session) -> Self {
        Self {
            tokens,
            pos: 0,
            term_env: Vec::new(),
            ...
            stop_at_field: false,
            stop_at_as: false,
            session_ptr: session as *mut owl_kernel::session::Session,
        }
    }
    ...
    fn session(&self) -> &mut owl_kernel::session::Session {
    ...
    fn parse_tactic(&mut self, intro_count: &mut usize) -> Result<Tactic, ParseError> {
        ...
        if self.consume_ident("tactic") {
            let name = self.expect_ident("expected tactic name after 'tactic'")?;
            return Ok(Tactic::Custom(name));
        }
        Err(self.error_here("expected tactic: 'exact', 'intro', 'apply', 'assumption', 'reflexivity', 'symmetry', 'split', 'constructor', 'destruct', 'transitivity', 'compute', 'trivial', 'omega', 'ring', 'field', 'group', 'ring with <term>', 'field with <term>', 'group with <term>', or 'tactic <name>'"))
    }
    ...
    fn parse_system(&mut self) -> Result<owl_kernel::syntax::System, ParseError> {
