...
use owl_kernel::syntax::{Datatype, Name, Pos, Term};
use owl_kernel::typechecker::infer_closed_dt;
...
impl ProgramParser {
    ...
    pub fn sync_from_env(&mut self, env: &owl_kernel::env::Env) {
...
pub fn typecheck_program(
    src: &str,
    session: &mut owl_kernel::session::Session,
) -> Result<
    (
        Vec<owl_kernel::syntax::Datatype>,
        Vec<(String, owl_kernel::syntax::Term, owl_kernel::syntax::Term)>,
    ),
    String,
> {
    use owl_kernel::syntax::Datatype;
    use owl_kernel::typechecker::check_closed_dt;

    let decls = parse_program(src, session).map_err(|e| e.to_string())?;

    let mut dts: Vec<Datatype> = Vec::new();
    let mut defs: Vec<(String, owl_kernel::syntax::Term, owl_kernel::syntax::Term)> = Vec::new();
    ...
    for decl in decls {
        match decl {
            ...
            Decl::Data(dt) => {
                ...
                owl_kernel::syntax::check_datatype_positivity(&dt).map_err(|e| format!("{}", e))?;
                // Make the datatype available to all subsequent declarations.
                dts.push(dt);
            }
            Decl::DataMutual(new_dts) => {
                for dt in &new_dts {
                    owl_kernel::syntax::check_datatype_positivity(dt)
                        .map_err(|e| format!("{}", e))?;
                    dts.push(dt.clone());
                }
            }
                ...
                match ty_ty {
                    ...
                    other => {
                        return Err(format!(
                            ...
                            name,
                            owl_kernel::syntax::show_term(&[], &other)
                        ));
                    }
                }
