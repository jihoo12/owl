...
use crate::parser::{Decl, ProgramParser};
use owl_kernel::env::{Env, check_with_full_env, infer_with_full_env};
use owl_kernel::nbe::{nbe_eval, nbe_eval_with_globals, zonk};
use owl_kernel::session::Session;
use owl_kernel::syntax::{LevelExpr, Name, Term};
use owl_kernel::typechecker::errors::ContextualError;
use owl_kernel::typechecker::{Ctx, TypeError};

/// Initialize the tactic resolver. Must be called before any typechecking.
pub fn init_tactic_resolver() {
    owl_kernel::tactics::set_tactic_resolver(Box::new(crate::tactics::OwlTacticResolver));
}
...
impl fmt::Display for RunOutput {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            "{} : {} = {}",
            self.name,
            owl_kernel::syntax::show_term(&self.global_names, &self.ty),
            owl_kernel::syntax::show_term(&self.global_names, &self.value),
        )
    }
}
...
fn normalize_definition(env: &Env, name: &str, session: &mut Session) -> RunOutput {
    ...
    let globals = owl_kernel::env::build_definition_values(env, session);
    RunOutput {
        name: name.clone(),
        ty: zonk(ty, session),
        value: zonk(
...
fn prune_datatypes(
    ...
    let kept: Vec<owl_kernel::syntax::Datatype> = env.datatypes[start..]
        .iter()
...
fn process_data(
    dt: &owl_kernel::syntax::Datatype,
    env: &mut Env,
    session: &mut Session,
) -> Result<(), RunError> {
    // Check positivity before registering the datatype.
    owl_kernel::syntax::check_datatype_positivity(dt).map_err(|e| {
        RunError::Type(Box::new(owl_kernel::typechecker::TypeError::Other(
            format!("{}", e),
        )))
    })?;
    ...
    let param_ctx: owl_kernel::typechecker::Ctx = dt
        .params
        .iter()
        .rev()
        .map(|(pname, pty)| {
    ...
    for con in &dt.cons {
        for arg_ty in &con.arg_tys {
            owl_kernel::typechecker::check_dt(
                &env.datatypes,
                &param_ctx,
                arg_ty,
    ...
    owl_kernel::typechecker::check_sqcon_coherence(&env.datatypes, dt, session)
        .map_err(|e| RunError::Type(Box::new(e)))?;
    Ok(())
}
...
fn process_data_mutual(
    dts: &[owl_kernel::syntax::Datatype],
    env: &mut Env,
    session: &mut Session,
) -> Result<(), RunError> {
...
fn process_postulate(
    ...
    session.clear_nbe_cache();
    session.clear_reflection_results();
    owl_kernel::debug_log!("process_postulate '{}':", name);
...
pub(crate) fn process_def(
    ...
    session.clear_nbe_cache();
    session.clear_reflection_results();
    owl_kernel::debug_log!("process_def '{}':", name);
    if by_wf {
        owl_kernel::typechecker::termination::set_skip_guard(true, session);
    }
    owl_kernel::debug_log!(
        "process_def '{}' raw_ty: {}",
        name,
        owl_kernel::syntax::pretty::show_term(&[], ty)
    );
    ...
    let check_ty = owl_kernel::syntax::shift(1, 0, ty);
    ...
    let resolved_val =
        crate::tactics::resolve_tactics(&env.datatypes, val, &check_ty, &global_ctx, session)
    ...
    let prev_def =
        owl_kernel::typechecker::termination::set_current_def(Some(name.clone()), session);
    ...
    let result = check_with_full_env(env, &resolved_val, &check_ty, session).map_err(|e| {
        ...
        if from_tactic && !owl_kernel::debug::is_active() {
            RunError::Type(Box::new(TypeError::Other(format!(
    ...
    owl_kernel::typechecker::termination::set_current_def(prev_def, session);
    session.set_reflection_ctx(prev_ctx);
    if by_wf {
        owl_kernel::typechecker::termination::set_skip_guard(false, session);
    }
    result?;
