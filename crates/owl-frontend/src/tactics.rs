use owl_kernel::equality::EtaResult;
use owl_kernel::equality::definitionally_equal_ctx_r;
use owl_kernel::nbe::{Scope, nbe_eval, nbe_eval_ctx, quote};
use owl_kernel::session::Session;
use owl_kernel::syntax::{Datatype, Name, Term, beta, shift, show_term};
use owl_kernel::typechecker::{Ctx, TypeError, err_pos, infer_dt};
use std::sync::Arc;
...
impl<'a> TacticEngine<'a> {
    ...
    fn process_pending_goal(&mut self, session: &mut Session) -> Result<(), TypeError> {
        ...
        match pending {
            ...
            PendingGoal::TransitivityFirst { x, z, a_ty } => {
                ...
                let y = Term::PApp(
                    Arc::new(p1.clone()),
                    Arc::new(Term::TInterval(owl_kernel::interval::I::I1)),
                );
            ...
            PendingGoal::MultiGoal {
                ...
                if goals.is_empty() {
                    ...
                    let cases: Vec<owl_kernel::syntax::ElimCase> = results
                        .into_iter()
                        .enumerate()
                        .map(|(i, body)| {
    ...
    pub fn run_tactic(
        &mut self,
        tactic: &owl_kernel::syntax::Tactic,
        outer_ctx: &Ctx,
        session: &mut Session,
    ) -> Result<(), TypeError> {
        use owl_kernel::syntax::Tactic;
        ...
        match tactic {
            ...
            // ── field ─────────────────────────────────────────────────────
            Tactic::Field(field_term) => {
                let combined_ctx: Ctx = {
                    let mut c = self.tactic_ctx.clone();
                    c.extend_from_slice(outer_ctx);
                    c
                };
                let result = crate::field::prove(
                    self.dts,
                    &combined_ctx,
                    &self.goal_ty,
                    self.tactic_ctx.len(),
            ...
            Tactic::Custom(name) => {
                ...
                for level in 0..tactic_ctx_len {
                    scope = scope.extend(owl_kernel::nbe::Value::VNeutral(
                        owl_kernel::nbe::Neutral::nvar(level),
                    ));
                }

                // Evaluate the application.
                let result_val =
                    owl_kernel::nbe::eval_nbe(&scope, &globals, tactic_ctx_len, &app_term, session);
                ...
                let proof = match &result_val {
                    owl_kernel::nbe::Value::TermVal(t) => shift(tactic_ctx_len as i32, 0, t),
                    owl_kernel::nbe::Value::VNeutral(_) => {
...
// ---------------------------------------------------------------------------
// TacticResolver implementation
// ---------------------------------------------------------------------------

/// Wrapper that implements the kernel's TacticResolver trait.
#[derive(Debug)]
pub struct OwlTacticResolver;

impl owl_kernel::tactics::TacticResolver for OwlTacticResolver {
    fn resolve_tactics(
        &self,
        dts: &[Datatype],
        ctx: &Ctx,
        goal_ty: &Term,
        raw_goal_ty: &Term,
        tactics: &[owl_kernel::syntax::Tactic],
        session: &mut Session,
    ) -> Result<Term, TypeError> {
        let mut engine = TacticEngine::new(dts, goal_ty.clone(), raw_goal_ty.clone());
        for tac in tactics {
            engine.run_tactic(tac, ctx, session)?;
        }
        engine.into_term(session)
