...
use crate::nbe::value::NeutralInner;
use crate::nbe::{Globals, Neutral, Scope, Value, eval_nbe};
use crate::session::Session;
use crate::syntax::{Datatype, Name, Term, shift, subst};
use crate::typechecker::{Ctx, TypeError, check, check_dt, infer, infer_dt};
...
impl Env {
    ...
    pub fn find_instance(&self, target_ty: &Term, session: &mut Session) -> Option<Term> {
        let target_nf = crate::nbe::nbe_eval_ctx(0, target_ty, session);
        let empty_ctx: Ctx = Vec::new();
        for (_name, inst_ty, inst_val) in &self.instances {
            let inst_ty_nf = crate::nbe::nbe_eval_ctx(0, inst_ty, session);
            if crate::equality::definitionally_equal_ctx_r(
                &empty_ctx,
                &inst_ty_nf,
                &target_nf,
                session,
            ) == crate::equality::EtaResult::Equal
            {
