...
macro_rules! debug_log {
    ($($arg:tt)*) => {
        if $crate::debug::is_active() {
            eprintln!("{}{}", $crate::debug::indent_str(), format!($($arg)*));
        }
    };
}
...
macro_rules! debug_scope {
    ($($arg:tt)*) => {
        let _debug_scope = if $crate::debug::is_active() {
            $crate::debug::DebugScope::new(&format!($($arg)*))
        } else {
            $crate::debug::DebugScope::inactive()
        };
    };
}
