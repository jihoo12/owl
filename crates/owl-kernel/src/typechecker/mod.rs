...
use crate::equality::{EtaResult, definitionally_equal_ctx_r};
use crate::interval::{I, Literal};
use crate::nbe::{nbe_eval, nbe_eval_ctx};
use crate::session::Session;
use crate::syntax::{Datatype, ElimCase, LevelExpr, Name, Term, beta, shift, show_term, subst};
use crate::syntax::{is_bot_dnf, is_top_dnf};
...
fn infer_dt_inner(
    ...
    match t {
        ...
        Term::TSqCon(d, sc, args, r, s) => {
            ...
            let mut is_endpoint = |t: &Term| -> bool {
                match nbe_eval(t, session) {
                    Term::TInterval(i) => {
                        let dnf = crate::interval::eval_interval(&i);
                        dnf == crate::interval::dnf_bot() || dnf == crate::interval::dnf_top()
                    }
                    Term::TCube(d) => {
                        d == crate::interval::dnf_bot() || d == crate::interval::dnf_top()
                    }
                    _ => false,
                }
            };
        ...
        Term::TElim(motive, cases, scrut) => {
            ...
            if !crate::typechecker::termination::should_skip_guard(session) {
                let def_idx = crate::typechecker::termination::current_def(session)
                    .and_then(|dn| ctx.iter().position(|(n, _)| n == &dn).map(|p| p as i32));
                match crate::typechecker::termination::check_guard(&d, cases, def_idx) {
                    crate::typechecker::termination::GuardStatus::Ok => {}
                    crate::typechecker::termination::GuardStatus::Violation { case, msg } => {
        ...
        Term::TUnify(a, bx) => {
            ...
            match eq {
                crate::equality::EtaResult::Equal => Ok(Term::TData("Unit".to_string(), vec![])),
...
fn check_dt_inner(
    ...
    match t {
                    ...
                    if let Some(ref return_args) = sig.return_args {
                        ...
                        if has_repeated {
                            let substituted: Vec<Term> = return_args
                                .iter()
                                .map(|arg| {
                                    crate::syntax::subst_params(num_params, &param_terms, arg)
                                })
                                .collect();
        ...
        Term::TBy(tactics) => {
            ...
            let resolver = crate::tactics::tactic_resolver()
                .ok_or_else(|| TypeError::Other("Tactic resolver not available".into()))?;
            let proof = resolver.resolve_tactics(dts, ctx, &goal_ty, ty, tactics, session)?;
            let prev = crate::typechecker::termination::should_skip_guard(session);
            if tactics
                .iter()
                .any(|t| matches!(t, crate::syntax::Tactic::Ring(_)))
            {
