...
use crate::syntax::{Datatype, Term, subst};
        ...
        Term::TPCon(d, pc, args, r) => {
            ...
            let (is_i0, is_i1) = match &r_nf {
                Term::TInterval(i) => {
                    ...
                    (
                        dnf == crate::interval::dnf_bot(),
                        dnf == crate::interval::dnf_top(),
                    )
                }
                Term::TCube(d) => (
                    d == &crate::interval::dnf_bot(),
                    d == &crate::interval::dnf_top(),
                ),
                _ => (false, false),
            };
            if is_i0 || is_i1 {
                    ...
                    if let Some(dt) = dts.iter().find(|dt| &dt.name == d) {
                        ...
                        if let Some(sig) = dt.find_pcon(pc) {
                            let is_i0 = match &r_nf {
                                Term::TInterval(i) => {
                                    crate::interval::eval_interval(i) == crate::interval::dnf_bot()
                                }
                                _ => false,
                            };
                            let face = if is_i0 { &sig.face0 } else { &sig.face1 };
