...
use crate::nbe::{meta_mentions, nbe_eval, nbe_eval_ctx, try_solve_meta};
use crate::session::Session;
use crate::syntax::{Ctx, LevelExpr, Name, Term, beta, shift};
...
pub fn term_size(t: &Term) -> usize {
    match t {
        ...
        Term::TUnify(a, bx) => 1 + term_size(a) + term_size(bx),
        Term::TGetContext => 1,
...
pub fn reduce_papp_by_type(ctx: &Ctx, p: &Term, r: &Term, session: &mut Session) -> Option<Term> {
    match infer_ty(ctx, p, session) {
        Some(Term::TPath(_, u, v)) => {
            ...
            if d == crate::interval::dnf_bot() {
                Some(nbe_eval_ctx(ctx.len(), &u, session))
            } else if d == crate::interval::dnf_top() {
