...
- [ ] **Nested constructor patterns** — e.g. `suc (suc zero)` matching a literal 2 (requires a full pattern AST rather than the current flat-binder matching). *(🟡 — meaningful ergonomics win, moderate implementation cost.)* **Phase 1 (parser-side) is DONE; Phase 2 (HIT-case refinement, kernel change) remains.**
